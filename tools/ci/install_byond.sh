...
then
  ...
  $(curl --fail -H "User-Agent: EmeraldSummit/1.0 CI Script" "http://www.byond.com/download/build/${BYOND_MAJOR}/${BYOND_MAJOR}.${BYOND_MINOR}_byond_linux.zip" -o byond.zip)
  ...
  then
    ...
    curl --fail -H "User-Agent: EmeraldSummit/1.0 CI Script" ${FALLBACK_URL} -o byond.zip
  else
