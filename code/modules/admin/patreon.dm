...
//		return
	if(client.prefs.anonymize == TRUE)
		if(alert(src, "Disable Anonymize? (Not Recommended)", "EMERALD SUMMIT", "YES", "NO") == "YES")
		...
		maintaining privacy from other BYOND users.", "EMERALD SUMMIT", "YES", "NO") == "YES")
			if(GLOB.respawncounts[client.ckey])
				to_chat(src, span_warning("You have already spawned."))
				return
...
/mob/dead/new_player/proc/register_patreon()
	...
	if(client)
		if(client.patreonlevel())
			return
	var/name = input("Enter your patreon DISPLAY NAME exactly as it appears on Patreon.","EMERALD SUMMIT") as text|null
	if(!name)
		return
	var/email = input("Enter your patreon EMAIL ADDRESS exactly as it appears on Patreon.","EMERALD SUMMIT") as text|null
	if(!email)
		return
